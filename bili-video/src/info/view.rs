...
mod tests {
    ...
    fn test_deserialize_video_view() {
        let json = include_str!("../../../tests/datas/video_view.json");
    ...
    async fn test_get_video_view() {
        ...
        fs::write("../tests/datas/video_view.json", &json)
            .await
            .unwrap();

        let view = serde_json::from_str::<BiliResponse<VideoView>>(&json)
            .unwrap()
            .data()
            .unwrap();

        assert_eq!(view.bvid, BVID);
    }
}
