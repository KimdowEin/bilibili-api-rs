...
use crate::{
    error::Error,
    model::{response::BiliResponse, video::stream::{VideoStream, VideoStreamOld}},
...
pub async fn get_video_stream_old(session: &Session, query: VideoStreamQuery)->Result<VideoStreamOld,Error>{
    let url = format!(
        "{}?{}",
        VIDEO_STREAM_URL,
...
/// 获取视频流地址(Dash格式)
pub async fn get_video_stream(session: &Session, query: VideoStreamQuery)->Result<VideoStream,Error>{
    let url = format!(
        "{}?{}",
        VIDEO_STREAM_URL,
        query.sign(&session.mixin_key().await)?
...
mod tests {
    ...
    use crate::{model::video::format::{Fnval, Qn}, query::video::info::VideoQuery, service::video::get_video_cids};
    ...
    async fn test_get_video_stream() {
        ...
        let query = VideoStreamQuery::new(query,cid,Some(Qn::FHD),Some(Fnval::DASH | Fnval::HDR),None,None);
